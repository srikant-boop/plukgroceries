...
import { brandSlug, brands, getBrand } from "@/lib/brands";
...
export default async function BrandPage({
  ...
  const branded = products.filter(
    (p) => p.brand && brandSlug(p.brand) === brand.slug,
  );
    ...
    <article>
      <header className="border-b border-line pb-10 mb-12 grid gap-6 sm:grid-cols-[1fr_auto] sm:items-end">
        <div>
          ...
          {brand.tagline && (
            <p className="text-muted text-lg">{brand.tagline}</p>
          )}
        </div>
        <div className="flex flex-col gap-3 sm:items-end">
          ...
          {brand.location && (
            <p className="text-sm text-muted sm:text-right">{brand.location}</p>
          )}
        </div>
      </header>
      ...
      {brand.location && (
        <section className="mb-16">
          ...
          <div className="aspect-[3/1] overflow-hidden border border-line bg-surface">
            <iframe
              src={`https://www.google.com/maps?q=${encodeURIComponent(brand.location)}&output=embed`}
      ...
      {branded.length > 0 && (
        <section>
          <div className="mb-6 flex items-baseline justify-between border-b border-line pb-3">
            <h2 className="text-2xl">{brand.name} at Pluk</h2>
            <span className="eyebrow">
              {branded.length} {branded.length === 1 ? "item" : "items"}
            </span>
          </div>
